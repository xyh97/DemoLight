...
class FormulaAgent(Agent):
    ...
    def __init__(self, dic_agent_conf, dic_traffic_env_conf, dic_path, cnt_round):
        ...
        lane_num = dic_traffic_env_conf["num_lanes"]
        self.list_vehicles_count_for_phase = np.zeros((self.n_segments, lane_num * 4))
        self.list_fixed_time = np.zeros((self.n_segments, dic_traffic_env_conf["num_phases"]))
        self.road_map = {
            ('road_0_1_0', 'road_1_1_0'): 0,
            ('road_2_1_2', 'road_1_1_2'): 1,
            ('road_1_2_3', 'road_1_1_3'): 2,
            ('road_1_0_1', 'road_1_1_1'): 3,
            ('road_0_1_0', 'road_1_1_1'): 4,
            ('road_2_1_2', 'road_1_1_3'): 5,
            ('road_1_2_3', 'road_1_1_0'): 6,
            ('road_1_0_1', 'road_1_1_2'): 7
        }
        # the traffic record can only record the first day
        traffic_json = json.load(open(os.path.join(self.dic_path['PATH_TO_WORK_DIRECTORY'], self.dic_agent_conf["TRAFFIC_FILE"])))

        for flow in traffic_json:
            road_id = self.road_map[tuple(flow['route'])]
            self.list_vehicles_count_for_phase[
                int(floor((int(flow['startTime']) % self.dic_agent_conf["DAY_TIME"]) / self.dic_agent_conf["UPDATE_PERIOD"])), road_id] = floor(
                self.dic_agent_conf["UPDATE_PERIOD"] / flow["interval"])

        print(self.list_vehicles_count_for_phase)
    ...
    def choose_action(self, count, state):
        ...
        if state["time_this_phase"][0] <= self.FIXED_TIME[state["cur_phase"][0]-1]:
            ...
            self.action = (self.action + 1) % dic_traffic_env_conf["num_phases"]
    ...
    def get_phase_split(self, vehicles_count_for_phase):
        ...
        if self.n_segments == 1:
            ...
            PHF = 1
            vc = 1
        N = self.dic_traffic_env_conf["NUM_PHASES"]
