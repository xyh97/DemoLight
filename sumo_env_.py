...
if platform == "linux" or platform == "linux2":
    # this is linux
    try:
        if 'SUMO_HOME' not in os.environ.keys():
...
class Intersection:
    ...
    def __init__(self, light_id, list_vehicle_variables_to_sub, dic_sumo_env_conf):
        ...
        '''
        self.dic_traffic_env_conf = dic_sumo_env_conf
        ...
        #self.list_approachs = ["N", "E", "S", "W"]
        self.list_approachs = ["W", "E", "N", "S"]
        self.dic_approach_to_node = {"N": "{0}.nodeN".format(self.node_light),
                                     "E": "{0}.nodeE".format(self.node_light),
                                     "S": "{0}.nodeS".format(self.node_light),
                                     "W": "{0}.nodeW".format(self.node_light)}
        ...
        self.lane_direc += ["s" for i in range(dic_sumo_env_conf["LANE_NUM"]["STRAIGHT"])]
        ...
        self.dic_entering_approach_lanes = {"W": self.l_lane_ind, "E": self.l_lane_ind, "S": self.l_lane_ind, "N": self.l_lane_ind}
        self.dic_exiting_approach_lanes = {"W": self.l_lane_ind, "E": self.l_lane_ind, "S": self.l_lane_ind, "N": self.l_lane_ind}
        ...
        # generate signals TODO next change sumo here
        self.list_phases = dic_sumo_env_conf["PHASE"]
        self.dic_app_offset = {"N": 0, "E": 1, "S": 2, "W": 3}
    ...
    def get_reward(self, dic_reward_info):
        ...
        if self.dic_traffic_env_conf['REWARD_NORM']:
            ...
            reward = - 2 * dic_reward["sum_num_vehicle_been_stopped_thres1"] / (37 * len(self.dic_feature["lane_num_vehicle_been_stopped_thres1"])) + 1
        else:
            reward = 0
