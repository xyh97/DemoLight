...
class Critic(object):
    def __init__(self, sess, n_features, config, dic_traffic_env_conf, demo, lr=0.01):
        self.sess = sess
        self.config = config
    ...
    def learn(self, s, r, s_, a, next_a, probs):
    ...
    def loss(self):
        ...
        return self.ISWeights * tf.reduce_sum([l * λ for l, λ in zip([l_dq,  l_jeq, l_l2], self.config.LAMBDA)])
    ...
    # def train_Q_network(self, actor=None, pre_train=False, update=True):
    #     """
    #     :param pre_train: True means should sample from demo_buffer instead of replay_buffer
    #     :param update: True means the action "update_target_net" executes outside, and can be ignored in the function
    #     """
    ...
    #     state_batch = [data[0] for data in minibatch]
    #     action_batch = [data[1] for data in minibatch]
    #     behavior_prob_batch = [data[2] for data in minibatch]
    #     reward_batch = [data[3] for data in minibatch]
    #     next_state_batch = [data[4] for data in minibatch]
    #     done_batch = [data[5] for data in minibatch]
    #     demo_data = [data[6] for data in minibatch]
    #     n_step_reward_batch = [data[7] for data in minibatch]
    #     n_step_state_batch = [data[8] for data in minibatch]
    #     n_step_done_batch = [data[9] for data in minibatch]
    #     actual_n = [data[10] for data in minibatch]
    #
    #     # provide for placeholder，compute first
    #     q_next = self.q.eval(feed_dict={self.s: next_state_batch})
    #     q_target_next = self.q_target.eval(feed_dict={self.s: next_state_batch})
    ...
    #     n_step_y_batch = np.zeros((self.config.BATCH_SIZE, self.action_dim))
    #     td_error_batch = np.zeros((self.config.BATCH_SIZE, 1))
    #     actions = np.zeros((self.config.BATCH_SIZE, 1))
    #     for i in range(self.config.BATCH_SIZE):
    #         # state, action, reward, next_state, done, demo_data, n_step_reward, n_step_state, n_step_done = t
    #         temp = self.q.eval(feed_dict={self.s: state_batch[i].reshape((-1, self.state_dim))})[0]
    #         temp_0 = np.copy(temp)
    #         actions[i] = action_batch[i]
    #         # add 1-step reward
    #         if pre_train:
    #             action = np.argmax(q_next[i])
    #         else:
    #             _, probs = actor.choose_action(state_batch[i])
    #             next_action, _ = actor.choose_action(next_state_batch[i])
    #             v = np.sum(temp * probs[0])
    #             td_error_batch[i] = probs[0, action_batch[i]] / behavior_prob_batch[i] * (temp[action_batch[i]] - v)
    #             action = next_action
    #         # action = next_action_batch[i]
    #         temp[action_batch[i]] = reward_batch[i] + (1 - int(done_batch[i])) * self.config.GAMMA * q_target_next[i][action]
    #         y_batch[i] = temp
    ...
    #         self.sess.run(self.update_target_net)
    #
    #     return state_batch, actions, td_error_batch
