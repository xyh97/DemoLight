...
class AnonEnv:
    ...
    def load_flow(self, flowFile=None):
        ...
        self.eng.load_flow(os.path.join(self.path_to_work_directory, self.dic_traffic_env_conf["TRAFFIC_FILE"]))
        print("successfully load flowFile: ", self.dic_traffic_env_conf["TRAFFIC_FILE"])
    ...
    def bulk_log(self, cnt_round):

        for inter_ind in range(len(self.list_intersection)):
            path_to_log_file = os.path.join(self.path_to_log, "vehicle_inter_{0}_round_{1}.csv".format(inter_ind, cnt_round))
        ...
        # vol = script.get_total_traffic_volume(self.dic_traffic_env_conf["TRAFFIC_FILE"])
